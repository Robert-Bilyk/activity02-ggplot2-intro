...
Gentoo species appear to have the lngest flippers overall as the mean and q1 and q3 appear to be quite longer than chinstrap or adelie species. Adelie and chinstrap appear somewhat similar although chinstrap is slightly longer.
...
The boxplot easily lets you see the mean and any major outliers in the dataset. Jitterplots do not easily let you see this information however, it does let you better view patterns in each variable.
...
The md file looks more professional overall and is easier to read.
...
I prefer the boxplot going down first. when placing the jitter down first, the boxplot covers a lot of the points making it hard to see. 
...
When placing the aestheics in he ggplot, it then applies to all of the geom statements after it. However, when putting it only in geom statments, it then only applies to that one plot.
...
ggplot(data=penguins,mapping=aes(x=bill_length_mm,position=sex,fill=sex)) +
  geom_boxplot()
```


```{r bill_length_by_island}
ggplot(data=penguins,mapping=aes(x=bill_length_mm,position=island,fill=island)) +
  geom_boxplot()
```
